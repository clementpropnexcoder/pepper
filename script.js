...
(function () {
  ...
  const revealEls = document.querySelectorAll(
    ".split-media, .split-body, .roe .wrap, .section-head, .award-card, .step, .contact-intro, .lead-form, .reviews-head, .review, .listings-text, .film-head, .film-player, .spot-head, .spot-safety, .spot-facts, .spot-video, .spot-savings"
  );
  revealEls.forEach(function (el) { el.classList.add("reveal"); });
  // Stagger grouped items so they cascade in rather than snapping together.
  [".award-card", ".step", ".review", ".fact", ".saving"].forEach(function (sel) {
